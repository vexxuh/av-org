...
const EditLocation: React.FC<EditLocationProps> = ({ location }) => {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values: FormValues) => {
    try {
      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}${Paths.LOCATION}/${location?.id}`,
        {
          ...values,
          user_id: user?.id,
    ...
    <Modal onClose={handleClose}>
      <div className="bg-white p-5 rounded-lg w-[700px] text-black flex flex-col gap-10 py-10">
        <header className="flex items-center justify-center w-full">
          <h1 className="text-2xl font-bold w-full">Edit Location</h1>
